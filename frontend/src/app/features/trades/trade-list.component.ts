...
import { Trade, TradeQuery, OutcomeTag, EmotionalState } from '../../shared/models/trade.model';
  ...
  template: `
    <div class="page">
      ...
      <div class="filter-bar">
        <select [(ngModel)]="filters.outcomeTag" (change)="applyFilters()" class="filter-select">
        ...
        <select [(ngModel)]="filters.emotionalState" (change)="applyFilters()" class="filter-select">
        ...
        <select [(ngModel)]="filters.setupType" (change)="applyFilters()" class="filter-select">
        ...
        <select [(ngModel)]="slFilter" (change)="applySlFilter()" class="filter-select">
          <option value="">SL — All</option>
          <option value="breached">SL Breached</option>
          <option value="respected">SL Respected</option>
        </select>
        <input [(ngModel)]="filters.instrument" (input)="applyFilters()" placeholder="Instrument..." class="filter-input" />
      ...
      <div class="table-wrapper">
        <table class="trade-table" *ngIf="trades().length; else empty">
          ...
          <tbody>
            <tr *ngFor="let t of trades()" [class.sl-breach]="!t.slRespected">
              ...
              <td>
                <span *ngIf="t.actualRR" [class.rr-good]="t.actualRR>=1" [class.rr-bad]="t.actualRR<1">{{ t.actualRR | number:'1.2-2' }}</span>
                <span *ngIf="!t.actualRR" class="muted">{{ t.plannedRR | number:'1.2-2' }}p</span>
              </td>
              <td [class.positive]="(t.pnlAbsolute||0)>=0" [class.negative]="(t.pnlAbsolute||0)<0" class="pnl-cell">
                {{ t.pnlAbsolute != null ? ((t.pnlAbsolute>=0?'+':'')+'₹'+(t.pnlAbsolute|number:'1.0-0')) : '—' }}
              </td>
              <td><span class="setup-badge">{{ t.setupType }}</span></td>
              <td><span class="emotion-badge" [class]="emotionClass(t.emotionalState)">{{ t.emotionalState }}</span></td>
              <td>
                <span *ngFor="let tag of (t.tags||[]).slice(0,2)" class="tag">{{ tag }}</span>
              </td>
              <td><span class="status-badge" [class]="statusClass(t.outcomeTag)">{{ t.outcomeTag }}</span></td>
              <td>
        ...
        <ng-template #empty>
          <div class="empty-state">
            ...
            <a routerLink="/trades/new" class="btn-primary">Log your first trade</a>
          </div>
        </ng-template>
      </div>
      ...
      <div class="pagination" *ngIf="totalPages() > 1">
        <button (click)="prevPage()" [disabled]="page()===0" class="page-btn">← Prev</button>
        <span class="page-info">Page {{ page()+1 }} of {{ totalPages() }}</span>
        <button (click)="nextPage()" [disabled]="page()===totalPages()-1" class="page-btn">Next →</button>
      </div>
    </div>
  `,
  styles: [`
    ...
    .btn-ghost { background: none; border: 1px solid #1e2433; color: #64748b; padding: 8px 14px; border-radius: 8px; cursor: pointer; font-size: 13px; &:hover { border-color: #ef4444; color: #ef4444; } }

    .table-wrapper { overflow-x: auto; }
    .trade-table {
      width: 100%; border-collapse: collapse; font-size: 13px;
      th { padding: 10px 12px; text-align: left; color: #475569; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 1px solid #1e2433; white-space: nowrap; }
      td { padding: 12px 12px; border-bottom: 1px solid #111827; color: #94a3b8; vertical-align: middle; }
      tr:hover td { background: rgba(30,36,51,0.4); }
      tr.sl-breach td { border-left: 2px solid #ef4444; }
    }

    .trade-id { font-family: monospace; font-size: 11px; color: #475569; }
    .date-cell { white-space: nowrap; color: #64748b; }
    .instrument-tag { font-weight: 700; color: #e2e8f0; background: #1e2433; padding: 3px 8px; border-radius: 4px; font-size: 12px; }
    .type-badge { font-size: 11px; color: #64748b; background: #111827; padding: 2px 6px; border-radius: 4px; }
    .dir-cell { font-weight: 700; font-size: 12px; &.buy { color: #22c55e; } &.sell { color: #ef4444; } }
    .sl-cell { color: #f59e0b; }
    .rr-good { color: #22c55e; font-weight: 600; }
    .rr-bad { color: #ef4444; font-weight: 600; }
    .muted { color: #475569; }
    .pnl-cell { font-weight: 600; white-space: nowrap; &.positive { color: #22c55e; } &.negative { color: #ef4444; } }
    .setup-badge { font-size: 11px; color: #7c3aed; background: rgba(124,58,237,0.1); padding: 2px 7px; border-radius: 4px; }
    .emotion-badge { font-size: 11px; padding: 2px 7px; border-radius: 4px;
      ...
      &.hesitation { color: #94a3b8; background: rgba(148,163,184,0.1); }
      &.overconfident { color: #fb923c; background: rgba(251,146,60,0.1); }
    }
    .tag { font-size: 11px; color: #475569; background: #111827; padding: 2px 6px; border-radius: 4px; margin-right: 4px; }
    ...
    .action-btn { color: #3b82f6; font-size: 12px; text-decoration: none; &:hover { text-decoration: underline; } }
    .positive { color: #22c55e; } .negative { color: #ef4444; }

    .empty-state { text-align: center; padding: 60px; color: #64748b; p { margin-bottom: 16px; } }
    .pagination { display: flex; align-items: center; gap: 16px; justify-content: center; margin-top: 24px; }
    .page-btn { background: #0d1117; border: 1px solid #1e2433; color: #94a3b8; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 13px; &:disabled { opacity: 0.4; cursor: not-allowed; } &:hover:not(:disabled) { border-color: #3b82f6; color: #3b82f6; } }
...
export class TradeListComponent implements OnInit {
  ...
  applySlFilter() { this.applyFilters(); }
  ...
  emotionClass(e: string) { return e?.toLowerCase() || ''; }
