...
@Component({
    ...
    <div class="page">
      ...
      <div class="kpi-grid" *ngIf="analytics()">
        <div class="kpi-card">
          ...
          <span class="kpi-value" [class.positive]="analytics()!.totalPnl >= 0" [class.negative]="analytics()!.totalPnl < 0">
      ...
      <div class="content-grid">
        <!-- Recommendations / Pattern Alerts -->
        <div class="card alerts-card" *ngIf="analytics() && analytics()!.recommendations.length">
        ...
        <div class="card">
          ...
          <div class="trades-list" *ngIf="recentTrades().length; else noTrades">
            <div *ngFor="let trade of recentTrades()" class="trade-row">
              ...
              <div class="trade-dir" [class.buy]="trade.direction==='BUY'" [class.sell]="trade.direction==='SELL'">
                {{ trade.direction }}
              </div>
              <div class="trade-pnl" [class.positive]="(trade.pnlAbsolute || 0) >= 0" [class.negative]="(trade.pnlAbsolute || 0) < 0">
                {{ trade.outcomeTag === 'OPEN' ? 'OPEN' : (((trade.pnlAbsolute || 0) >= 0 ? '+' : '') + '₹' + (trade.pnlAbsolute || 0 | number:'1.0-0')) }}
              </div>
              <div class="trade-tag" [class]="tagClass(trade.outcomeTag)">{{ trade.outcomeTag }}</div>
            </div>
          </div>
          <ng-template #noTrades>
            <div class="empty-state">No trades yet. <a routerLink="/trades/new">Log your first trade →</a></div>
          </ng-template>
        </div>
  ...
  styles: [`
    ...
    .card-title { font-size: 14px; font-weight: 600; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px; margin: 0 0 16px; }
    .card-link { font-size: 13px; color: #3b82f6; text-decoration: none; &:hover { text-decoration: underline; } }
    ...
    .insight-item {
      ...
      font-size: 13px;
      line-height: 1.5;
      &.warning { background: rgba(245,158,11,0.08); border-left: 3px solid #f59e0b; color: #fbbf24; }
      &.danger { background: rgba(239,68,68,0.08); border-left: 3px solid #ef4444; color: #f87171; }
    }
    ...
    .trade-type-badge { font-size: 11px; color: #64748b; background: #1e2433; padding: 2px 6px; border-radius: 4px; }
    .trade-dir { font-size: 12px; font-weight: 700; letter-spacing: 0.5px; &.buy { color: #22c55e; } &.sell { color: #ef4444; } }
    .trade-pnl { font-size: 14px; font-weight: 600; min-width: 80px; text-align: right; &.positive { color: #22c55e; } &.negative { color: #ef4444; } }
    .trade-tag { font-size: 11px; font-weight: 600; padding: 3px 8px; border-radius: 4px;
      &.profit { background: rgba(34,197,94,0.12); color: #22c55e; }
      &.loss { background: rgba(239,68,68,0.12); color: #ef4444; }
      &.open { background: rgba(59,130,246,0.12); color: #3b82f6; }
      &.breakeven { background: rgba(148,163,184,0.12); color: #94a3b8; }
    }

    .empty-state { text-align: center; padding: 32px; color: #64748b; font-size: 14px; a { color: #3b82f6; } }
    ...
    .setup-item { font-size: 13px; padding: 8px 12px; border-radius: 6px;
      &.positive { background: rgba(34,197,94,0.08); color: #4ade80; border-left: 2px solid #22c55e; }
...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.tradeService.queryTrades({ limit: 10, sortBy: 'tradeDate', sortDir: 'desc' })
      .subscribe(trades => this.recentTrades.set(trades));
  }

  gradeClass(grade: string): string {
    const map: Record<string, string> = { A: 'positive', B: 'positive', C: '', D: 'negative', F: 'negative' };
    return map[grade] || '';
  }

  tagClass(tag: string): string { return tag?.toLowerCase() || ''; }
}
