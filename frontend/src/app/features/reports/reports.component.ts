...
@Component({
    ...
    <div class="report-gen-card">
      <div class="rg-header">
        <div class="rg-title-row">
          ...
          <div>
            <h3 class="rg-title">Generate Custom Report</h3>
            <p class="rg-sub">
              Download PDF or send to your email for any date range
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="rg-body">
        <div class="rg-presets">
          <span class="rg-preset-label">Quick:</span>
          <button
        ...
        <div class="rg-actions">
          ...
          >
            {{
              generating() === "download" ? "Generating..." : "⬇ Download PDF"
            }}
          </button>
          <button
            class="btn-email"
            (click)="emailAndDownload()"
            [disabled]="generating() || !fromDate || !toDate"
          >
            {{ generating() === "email" ? "Sending..." : "✉ Email + Download" }}
          </button>
        </div>

        <div class="rg-note">
          <span>✉ Email + Download</span> sends to
          <strong>{{ userEmail() }}</strong> and downloads simultaneously.
        </div>

        <div class="rg-error" *ngIf="reportError()">{{ reportError() }}</div>
        <div class="rg-success" *ngIf="reportSuccess()">
          {{ reportSuccess() }}
        </div>
      </div>
    </div>
      ...
      <div class="report-body" *ngIf="report()">
        <div class="report-header-block">
          <div class="rh-stat">
            ...
            <span
              class="rhv"
              [class.pos]="report()!.totalPnl >= 0"
              [class.neg]="report()!.totalPnl < 0"
            >
              {{ report()!.totalPnl >= 0 ? "+" : "" }}₹{{
                report()!.totalPnl | number: "1.0-0"
              }}
            </span>
          </div>
          <div class="rh-stat">
            ...
            <span class="rhv" [class.pos]="report()!.winRate >= 50"
              >{{ report()!.winRate | number: "1.1-1" }}%</span
            >
          </div>
          <div class="rh-stat">
          ...
          <div class="rh-stat">
            ...
            <span
              class="rhv"
              [class.pos]="
                report()!.disciplineGrade === 'A' ||
                report()!.disciplineGrade === 'B'
              "
              [class.neg]="
                report()!.disciplineGrade === 'D' ||
                report()!.disciplineGrade === 'F'
              "
            >
          ...
          <div class="rh-stat">
            ...
            <span class="rhv"
              >₹{{ report()!.expectancy | number: "1.0-0" }}</span
            >
          </div>
          <div class="rh-stat">
            <span class="rhl">Avg Win</span>
            <span class="rhv pos"
              >₹{{ report()!.avgProfitPerWin | number: "1.0-0" }}</span
            >
          </div>
          <div class="rh-stat">
            <span class="rhl">Avg Loss</span>
            <span class="rhv neg"
              >₹{{ report()!.avgLossPerLoss | number: "1.0-0" }}</span
            >
          </div>
        </div>

        <div
          class="insights-block"
          *ngIf="
            report()!.recommendations.length ||
            report()!.repeatingMistakes.length ||
            report()!.bestSetups.length
          "
        >
          <h3 class="block-title">Agent Assessment</h3>
          <div
            *ngFor="let r of report()!.recommendations"
            class="insight warning"
          >
            {{ r }}
          </div>
          <div
            *ngFor="let m of report()!.repeatingMistakes"
            class="insight danger"
          >
            ✗ Mistake: {{ m }}
          </div>
          <div *ngFor="let s of report()!.bestSetups" class="insight success">
            ✓ Strong: {{ s }}
          </div>
          <div
            *ngFor="let b of report()!.worstBehaviors"
            class="insight danger"
          >
            ⚠ {{ b }}
          </div>
        ...
        <div class="discipline-block">
          ...
          <div *ngIf="report()!.disciplineBreaks.length; else noDiscipline">
            <div
              *ngFor="let d of report()!.disciplineBreaks"
          ...
          <ng-template #noDiscipline>
            <div class="clean-slate">
              ✓ No discipline breaks in this period. Solid work.
            </div>
          </ng-template>
        </div>
        ...
        <div class="trades-block">
          <div class="trades-block-header">
            ...
            <span class="trade-count-badge"
              >{{ filteredTrades().length }} trades</span
            >
          </div>

          <div
            class="table-wrapper"
            *ngIf="pagedTrades().length; else noTrades"
          >
            <table class="trade-table">
              ...
              <tbody>
                <tr *ngFor="let t of pagedTrades()">
                  ...
                  <td>
                    <span class="instr-tag">{{ t.instrument }}</span>
                  </td>
                  <td>
                    <span class="type-tag">{{ t.tradeType }}</span>
                  </td>
                  <td
                  ...
                  <td
                    [class.pos]="(t.pnlAbsolute || 0) >= 0"
                    [class.neg]="(t.pnlAbsolute || 0) < 0"
                    class="pnl-cell"
                  >
                    {{
                      t.pnlAbsolute != null
                  ...
                  <td
                    [class.rr-good]="(t.actualRR || 0) >= 1"
                    [class.rr-bad]="(t.actualRR || 0) < 1 && t.actualRR"
                  >
                    {{
                      t.actualRR
                  ...
                  <td>
                    <span class="setup-tag">{{ t.setupType }}</span>
                  </td>
                  <td>
                    <div class="tf-mini-list">
                      <span
                        *ngFor="let tf of (t.timeFrames || []).slice(0, 2)"
                        class="tf-mini"
                        >{{ tf }}</span
                      >
                      <span
                        *ngIf="(t.timeFrames || []).length > 2"
                        class="tf-more"
                        >+{{ (t.timeFrames || []).length - 2 }}</span
                      >
                    </div>
                  </td>
                  <td>
                    <span
                      class="status-tag"
                      [ngClass]="t.outcomeTag?.toLowerCase()"
                      >{{ t.outcomeTag }}</span
                    >
                  </td>
                  <td>
                    <a [routerLink]="['/trades', t.id]" class="view-link"
                      >View</a
                    >
                  </td>
                </tr>
          ...
          <div class="pagination" *ngIf="totalTradePagesCount() > 1">
            ...
            <div class="page-numbers">
              <button
                *ngFor="let p of visibleTradePages()"
            ...
            <button
              (click)="nextTradePage()"
              [disabled]="tradePage() === totalTradePagesCount() - 1"
              class="page-btn"
            >
              Next →
            </button>
            <button
              (click)="goToTradePage(totalTradePagesCount() - 1)"
              [disabled]="tradePage() === totalTradePagesCount() - 1"
              class="page-btn"
            >
              »
            ...
            <span class="page-info"
              >Page {{ tradePage() + 1 }} of {{ totalTradePagesCount() }} ·
              {{ pagedTrades().length }} of {{ filteredTrades().length }}</span
            >
          </div>
        </div>
      </div>
  ...
  styles: [
    `
      ...
      .trades-block-header .block-title {
        margin: 0;
      }
      .trade-count-badge {
        font-size: 12px;
...
export class ReportsComponent implements OnInit {
  ...
  generating = signal<"download" | "email" | null>(null);
  reportError = signal("");
  ...
  // computed() now correctly imported
  ...
  selectTab(key: string) {
    ...
    const obs =
      key === "today"
        ? this.tradeService.getDailyReport()
        : key === "monthly"
          ? this.tradeService.getMonthlyReport()
          : key === "yearly"
            ? this.tradeService.getYearlyReport()
    ...
    this.tradeService
      .getTradesInRange(range.from, range.to)
      .subscribe((trades) => {
  ...
  downloadPdf() {
    ...
    const url = `${environment.apiUrl}/reports/download?from=${this.fromDate}&to=${this.toDate}`;
    this.http.get(url, { responseType: "blob" }).subscribe({
      next: (blob) => {
        ...
        setTimeout(() => this.reportSuccess.set(""), 4000);
      },
      error: (err) => {
        this.generating.set(null);
        this.reportError.set(
          err?.error?.message || "Failed to generate PDF. Try again.",
        );
      },
    });
  }
    ...
    this.http
      ...
      .subscribe({
        next: (blob) => {
          ...
          this.generating.set(null);
          this.reportSuccess.set(
            `PDF downloaded ✓  Email sent to ${this.userEmail()}`,
          );
          setTimeout(() => this.reportSuccess.set(""), 5000);
        },
        error: (err) => {
          this.generating.set(null);
          this.reportError.set(
            err?.error?.message ||
              "Failed. Check your email settings in Profile.",
          );
        },
      });
  }
  ...
  updateTradePage() {
    ...
    this.pagedTrades.set(
      this.filteredTrades().slice(start, start + this.tradePageSize),
    );
  }
  ...
  totalTradePagesCount(): number {
    return Math.max(
      1,
      Math.ceil(this.filteredTrades().length / this.tradePageSize),
    );
  }
  ...
  prevTradePage() {
    if (this.tradePage() > 0) this.goToTradePage(this.tradePage() - 1);
  }
  nextTradePage() {
    if (this.tradePage() < this.totalTradePagesCount() - 1)
      this.goToTradePage(this.tradePage() + 1);
  }
}
