      ...
      <div class="auth-card">
        ...
        <div class="form">
          <div class="form-group">
            <label>Email</label>
            <input type="email" [(ngModel)]="email" class="form-input" placeholder="trader@example.com" />
          </div>
          <div class="form-group">
        ...
        <p class="auth-footer">New trader? <a routerLink="/auth/register">Create account</a></p>
      </div>
    </div>
  `,
  styles: [`
    .auth-shell { min-height: 100vh; background: #0a0e1a; display: flex; align-items: center; justify-content: center; }
    .auth-card { background: #0d1117; border: 1px solid #1e2433; border-radius: 16px; padding: 40px; width: 100%; max-width: 400px; }
    ...
    .btn-primary { background: #3b82f6; color: #fff; padding: 12px; border-radius: 8px; border: none; font-size: 15px; font-weight: 600; cursor: pointer; margin-top: 4px; &:hover { background: #2563eb; } &:disabled { opacity: 0.6; cursor: not-allowed; } }
    .auth-footer { font-size: 13px; color: #64748b; text-align: center; margin-top: 24px; a { color: #3b82f6; text-decoration: none; &:hover { text-decoration: underline; } } }
  `]
})
