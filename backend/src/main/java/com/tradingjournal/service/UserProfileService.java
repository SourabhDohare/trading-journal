...
public class UserProfileService {
    ...
    public UserProfileDTO.Response updateProfile(String userId, UserProfileDTO.UpdateRequest req) {
        ...
        if (req.getPrimaryStyle() != null)
            user.setPrimaryStyle(req.getPrimaryStyle());
        if (req.getMarketsTraded() != null)
            user.setMarketsTraded(req.getMarketsTraded());
        if (req.getPlatformsUsed() != null)
            user.setPlatformsUsed(req.getPlatformsUsed());
        if (req.getPrimaryBroker() != null)
            user.setPrimaryBroker(req.getPrimaryBroker());

        // ── Capital & risk ────────────────────────────────────────────────
        if (req.getTradingCapital() != null)
            user.setTradingCapital(req.getTradingCapital());
        if (req.getRiskPerTradePercent() != null)
            user.setRiskPerTradePercent(req.getRiskPerTradePercent());
        if (req.getMaxDrawdownTolerance() != null)
            user.setMaxDrawdownTolerance(req.getMaxDrawdownTolerance());
        if (req.getTargetMonthlyReturnPct() != null)
            user.setTargetMonthlyReturnPct(req.getTargetMonthlyReturnPct());
        if (req.getAvgTradesPerMonth() != null)
            user.setAvgTradesPerMonth(req.getAvgTradesPerMonth());

        // ── Goals ─────────────────────────────────────────────────────────
        if (req.getTradingGoal() != null)
            user.setTradingGoal(req.getTradingGoal());
        if (req.getBiggestWeakness() != null)
            user.setBiggestWeakness(req.getBiggestWeakness());
        if (req.getWhyImproving() != null)
            user.setWhyImproving(req.getWhyImproving());

        // ── Settings ──────────────────────────────────────────────────────
        if (req.getStrictMode() != null)
            user.setStrictMode(req.getStrictMode());
        if (req.getEmailNotifications() != null)
            user.setEmailNotifications(req.getEmailNotifications());
        if (req.getWeeklyReportEmail() != null)
            user.setWeeklyReportEmail(req.getWeeklyReportEmail());
