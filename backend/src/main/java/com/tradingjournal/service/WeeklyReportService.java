...
public class WeeklyReportService {
    ...
    public void sendWeeklyReports() {
        ...
        LocalDate weekEnd   = LocalDate.now().with(DayOfWeek.SUNDAY).minusWeeks(1); // last Sunday
        ...
        for (User user : users) {
            try {
                ...
                log.error("Failed to send weekly report for user {}: {}", user.getEmail(), e.getMessage());
            }
        }
    }

    // ── Per-user weekly report logic ────────────────────────────────────────
    private void sendWeeklyReportForUser(User user, LocalDateTime from, LocalDateTime to,
        ...
        if (weekTrades.isEmpty()) {
            // No trades this week — send motivational nudge email
            sendNoActivityEmail(user.getEmail(), userName, weekLabel);
        } else {
            // Has trades — compute stats and send full report with PDF
            ...
            try {
                ...
                String filename = "TradePulse_Report_" + weekStart.toString() + ".pdf";
                emailService.sendEmailWithAttachment(
                        user.getEmail(),
                        "📊 Your TradePulse Weekly Report — " + weekLabel,
                        htmlBody, pdf, filename);
            } catch (Exception e) {
                // Fall back to HTML-only if PDF fails
                log.warn("PDF generation failed for {}, sending HTML only: {}", user.getEmail(), e.getMessage());
    ...
    private void sendNoActivityEmail(String email, String name, String weekLabel) {
        ...
        String html = "<!DOCTYPE html><html><body style='font-family:Inter,sans-serif;"
                ...
                + "builds the pattern recognition that separates professional traders from amateurs. "
                + "Log your market observations, even if you didn't trade.</p>"
                + "</div>"
    ...
    private WeekStats computeStats(List<Trade> closed) {
        ...
        s.winRate = s.totalTrades > 0
                ...
                : BigDecimal.ZERO;
        s.totalPnl = closed.stream().map(Trade::getPnlAbsolute)
                .filter(Objects::nonNull).reduce(BigDecimal.ZERO, BigDecimal::add);
        s.bestTrade = closed.stream().map(Trade::getPnlAbsolute).filter(Objects::nonNull)
                .max(BigDecimal::compareTo).orElse(BigDecimal.ZERO);
        s.worstTrade = closed.stream().map(Trade::getPnlAbsolute).filter(Objects::nonNull)
                .min(BigDecimal::compareTo).orElse(BigDecimal.ZERO);
        s.avgRR = closed.stream().map(Trade::getActualRR).filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        if (!closed.isEmpty() && s.avgRR.compareTo(BigDecimal.ZERO) > 0) {
    ...
    private String buildReportHtml(String name, String weekLabel, WeekStats s, List<Trade> trades) {
        ...
        for (Trade t : trades) {
            String tPnlColor = t.getPnlAbsolute() != null && t.getPnlAbsolute().compareTo(BigDecimal.ZERO) >= 0
                    ? "#22c55e" : "#ef4444";
            String tPnl = t.getPnlAbsolute() != null
                    ? (t.getPnlAbsolute().compareTo(BigDecimal.ZERO) >= 0 ? "+" : "") + "₹" + t.getPnlAbsolute().toPlainString()
                    : "—";
            tradeRows.append("<tr>")
                    .append("<td style='padding:10px 8px;color:#94a3b8;border-bottom:1px solid #1e2433'>")
                    .append(t.getInstrument()).append("</td>")
                    .append("<td style='padding:10px 8px;color:#64748b;border-bottom:1px solid #1e2433'>")
                    .append(t.getTradeType()).append("</td>")
                    .append("<td style='padding:10px 8px;border-bottom:1px solid #1e2433;color:")
                    .append(t.getDirection() == Trade.Direction.BUY ? "#22c55e" : "#ef4444").append("'>")
                    .append(t.getDirection()).append("</td>")
                    .append("<td style='padding:10px 8px;color:").append(tPnlColor)
                    .append(";font-weight:700;border-bottom:1px solid #1e2433'>").append(tPnl).append("</td>")
                    .append("<td style='padding:10px 8px;border-bottom:1px solid #1e2433;color:#64748b'>")
                    .append(t.getOutcomeTag()).append("</td>")
                    .append("</tr>");
        }
        ...
        return "<!DOCTYPE html><html><body style='font-family:Inter,sans-serif;background:#0a0e1a;color:#e2e8f0;padding:0;margin:0'>"
                ...
                + "<div style='text-align:center;margin-bottom:32px'>"
                + "<span style='font-size:28px;font-weight:900;background:linear-gradient(135deg,#3b82f6,#8b5cf6);"
                + "-webkit-background-clip:text;-webkit-text-fill-color:transparent'>TradePulse</span>"
                ...
                // Discipline alert
                + (s.slBreaches > 0
                    ? "<div style='margin-top:20px;background:rgba(239,68,68,.08);border-left:3px solid #ef4444;"
                      + "border-radius:0 8px 8px 0;padding:12px 16px'>"
                      + "<p style='color:#f87171;margin:0;font-size:13px'>⚠ <strong>" + s.slBreaches
                      + " SL breach(es)</strong> detected this week. Review your trade journal for discipline patterns.</p></div>"
                    : "<div style='margin-top:20px;background:rgba(34,197,94,.08);border-left:3px solid #22c55e;"
                      + "border-radius:0 8px 8px 0;padding:12px 16px'>"
                      + "<p style='color:#4ade80;margin:0;font-size:13px'>✓ Perfect discipline this week — no SL breaches. Keep it up.</p></div>")
                + "</div>"
                + "<div style='text-align:center;margin-bottom:20px'>"
                + "<a href='https://trading-journal-plum-gamma.vercel.app/analytics' "
                + "style='display:inline-block;background:#3b82f6;color:#fff;padding:12px 28px;"
                + "border-radius:8px;text-decoration:none;font-weight:700;font-size:14px'>View Full Analytics →</a></div>"
                + "<p style='text-align:center;color:#334155;font-size:12px'>"
                + "Full PDF report is attached · <a href='https://trading-journal-plum-gamma.vercel.app/profile' style='color:#475569'>Manage preferences</a>"
                + "</p></div></body></html>";
    }
    ...
    private byte[] buildReportPdf(String name, String weekLabel, WeekStats s, List<Trade> trades)
        ...
        String[] kpiValues  = {
            ...
            s.winRate + "%",
            String.valueOf(s.totalTrades),
            "+₹" + s.bestTrade.toPlainString(),
            "₹" + s.worstTrade.toPlainString(),
        ...
        String[] cols = {"Instrument", "Type", "Dir", "Entry", "P&L", "Status"};
        for (String col : cols) {
